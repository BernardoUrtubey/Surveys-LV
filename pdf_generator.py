...
def room_height(items):
    """Estimate height a room will occupy in points."""
...
def generate_survey_pdf(data, lang='es'):
    ...
    if lang == 'en':
        rooms = translate_rooms(rooms)
        crates = translate_crates(crates)
...
def translate_to_english(texts):
    """Translate a list of texts to English using a simple dictionary + API fallback."""
    # Common moving items ES->EN dictionary
    DICT = {
        # Rooms
        'Cocina': 'Kitchen', 'Lavadero': 'Laundry', 'Garage': 'Garage',
        'Comedor': 'Dining Room', 'Living': 'Living Room', 'Baño': 'Bathroom',
        'Dormitorio Principal': 'Master Bedroom', 'Dormitorio Niña': "Girl's Bedroom",
        'Dormitorio Niño': "Boy's Bedroom", 'Dormitorio 2': 'Bedroom 2',
        'Dormitorio 3': 'Bedroom 3', 'Estudio': 'Study', 'Terraza': 'Terrace',
        'Balcón': 'Balcony', 'Depósito': 'Storage',
        # Kitchen
        'Refrigerador': 'Refrigerator', 'Freezer': 'Freezer',
        'Lavavajilla': 'Dishwasher', 'Microwave': 'Microwave',
        'Utensilios de cocina': 'Kitchen utensils', 'Vajilla': 'Dishes',
        'Cubiertos': 'Cutlery', 'Mantelería': 'Table linen',
        'Electrodomésticos pequeños': 'Small appliances', 'Cristalería': 'Glassware',
        'Heladera': 'Refrigerator', 'Cocina/Horno': 'Stove/Oven',
        # Laundry
        'Lavarropas': 'Washing machine', 'Secarropas': 'Dryer',
        'Utensilios de limpieza': 'Cleaning supplies',
        'Utensilios de planchado': 'Ironing supplies', 'Plancha': 'Iron',
        # Garage
        '2 bicicletas': '2 bicycles', 'Bicicleta': 'Bicycle',
        'Scooter eléctrico': 'Electric scooter', 'Mesa': 'Table',
        'Herramientas manuales': 'Hand tools', 'Herramientas': 'Tools',
        # Living/Dining
        'Mesa comedor': 'Dining table', '6 sillas': '6 chairs',
        'Sillas': 'Chairs', 'Silla': 'Chair', 'Cuadros': 'Artwork',
        'Adornos': 'Decorations', 'Mesa de arrime': 'Side table',
        'Adornos de pared': 'Wall decorations', 'Espejo': 'Mirror',
        'Sofás': 'Sofas', 'Sofá': 'Sofa', 'TV LED': 'LED TV',
        'Mueble TV (se desarma)': 'TV unit (disassembles)',
        'Mueble TV': 'TV unit', 'Mesitas laterales': 'Side tables',
        'Mesas de centro': 'Coffee tables', 'Alfombra': 'Rug',
        'Cortinas': 'Curtains', 'Lámpara': 'Lamp', 'Lámparas': 'Lamps',
        # Bedroom
        'Cama single': 'Single bed', 'Cama': 'Bed',
        'Cama Kingsize': 'Kingsize bed', 'Cama Queen': 'Queen bed',
        'Mesita lateral': 'Bedside table', 'Mesita de noche': 'Bedside table',
        '2 mesitas de noche': '2 bedside tables',
        'Cómoda': 'Dresser', 'Cabecera de cama': 'Headboard',
        '2 lámparas de noche': '2 bedside lamps',
        'Perchero': 'Coat rack', 'Efectos personales': 'Personal effects',
        'Ropa': 'Clothing', 'Ropa de cama': 'Bed linen',
        'Toallas': 'Towels', 'Zapatos': 'Shoes',
        'Mueble zapatero': 'Shoe rack', 'Adornos': 'Decorations',
        'Libros': 'Books', 'Almohadas': 'Pillows',
        'Juguetes': 'Toys', 'Mesa de arrime': 'Side table',
        # Crates
        'A medir': 'To be measured', 'Sin medidas': 'No measurements',
        'Trámite — sin medidas': 'No crate needed',
    }
    ...
    for text in texts:
        ...
        for k, v in DICT.items():
            if k.lower() == text.lower():
                ...
                break
        if not found:
            # Check if it starts with a number + known word
            # e.g. "6 m³ de cajas con efectos del hogar"
            if 'm³' in text or 'cajas' in text:
                results.append(text.replace('de cajas con efectos del hogar', 'boxes w/ household items')
                                   .replace('m³ de', 'm³'))
            else:
                results.append(text)  # Keep original if not found
    return results


def translate_rooms(rooms):
    """Translate room names and items to English."""
    translated = []
    for room in rooms:
        t_room = dict(room)
        # Translate room name
        names = translate_to_english([room.get('name', '')])
        t_room['name'] = names[0]
        # Translate items
        t_room['items'] = translate_to_english(room.get('items', []))
        translated.append(t_room)
    return translated


def translate_crates(crates):
    """Translate crate names and statuses to English."""
    translated = []
    for crate in crates:
        t_crate = dict(crate)
        names = translate_to_english([crate.get('name', ''), crate.get('status', '')])
        t_crate['name'] = names[0]
        t_crate['status'] = names[1]
        translated.append(t_crate)
    return translated


def translate_notes(notes):
    """Translate notes to English - basic common phrases."""
    NOTE_DICT = {
        'A confirmar': 'To be confirmed',
        'Franquicia: a confirmar.': 'Customs allowance: to be confirmed.',
        'Fecha de salida: a confirmar.': 'Departure date: to be confirmed.',
    }
    translated = []
    for note in notes:
        found = False
        for k, v in NOTE_DICT.items():
            if k.lower() in note.lower():
