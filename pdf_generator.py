...
def translate_with_claude(texts):
    """Use Claude API to translate a list of texts to English."""
    if not texts:
        return texts
    ...
    prompt = (
        ...
        "These are household/moving items, room names, or operational notes. "
...
def translate_data(data):
    """Translate all user-entered content to English using Claude API."""
    rooms = data.get('rooms', [])
    crates = data.get('crates', [])
    notes = data.get('notes', [])

    # Collect all texts to translate in one API call
    all_texts = []

    # Room names
    room_names = [r.get('name', '') for r in rooms]
    all_texts.extend(room_names)
    ...
    for r in rooms:
        items = r.get('items', [])
        room_items_flat.extend(items)
        room_items_counts.append(len(items))
    all_texts.extend(room_items_flat)

    # Crate names and statuses
    crate_names = [c.get('name', '') for c in crates]
    crate_statuses = [c.get('status', '') for c in crates]
    all_texts.extend(crate_names)
    all_texts.extend(crate_statuses)

    # Notes
    all_texts.extend(notes)

    # Translate everything at once
    translated = translate_with_claude(all_texts)

    # Rebuild structure
    idx = 0

    # Room names
    t_room_names = translated[idx:idx+len(room_names)]
    idx += len(room_names)

    # Room items
    t_rooms = []
    for i, room in enumerate(rooms):
        count = room_items_counts[i]
        t_items = translated[idx:idx+count]
        idx += count
        t_rooms.append({'name': t_room_names[i], 'items': t_items})

    # Crates
    t_crate_names = translated[idx:idx+len(crate_names)]
    idx += len(crate_names)
    t_crate_statuses = translated[idx:idx+len(crate_statuses)]
    idx += len(crate_statuses)
    t_crates = [{'name': t_crate_names[i], 'status': t_crate_statuses[i]} for i in range(len(crates))]

    # Notes
    t_notes = translated[idx:idx+len(notes)]

    return t_rooms, t_crates, t_notes
...
def generate_survey_pdf(data, lang='es'):
    ...
    # Auto-translate everything to English using Claude API
    if lang == 'en':
        rooms, crates, notes = translate_data({
            'rooms': rooms,
            'crates': crates,
            'notes': notes
        })
